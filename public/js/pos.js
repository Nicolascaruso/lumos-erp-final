...
$(document).on('change', '.payment_types_dropdown', function() {
    let value = $(this).val();
    let isPos = window.location.pathname
    isPos = isPos.split("/")

    if(isPos[1]){
        if(value == 'boleto' && isPos[1] == "pos"){
            swal("Atenção", "A emissão de boletos deverá ser feita em vendas/novo venda", "warning")
            $(this).val("cash").change()
        }
    }
...
function pos_discount(total_amount) {
    ...
    var price_total = $('span.price_total').text().replace(",", ".")
    price_total = parseFloat(price_total)

    if(discount > price_total){
        swal("Alerta", "O valor de desconto é maior que o de venda", "warning")
        $('span#total_discount').text(__currency_trans_from_en(0, false))
